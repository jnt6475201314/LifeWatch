...
- (void)setupUI
{
    ...
    [self.arrayRoot enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.arrayStartMonth addObject:obj[@"startMonth"]];
    }];
    
    self.arrayEndMonth= [NSMutableArray arrayWithArray:[self.arrayRoot firstObject][@"endMonth"]];
...
{
    if (component == 0) {
        self.arraySelected = self.arrayRoot[row][@"endMonth"];
...
- (void)selectedOk
{
    [self.delegate pickerMonth:self selectedStartMonth:self.monthStartStr selectedEndMonth:self.monthEndStr];
    [super selectedOk];
}
...
{
    if (!_arrayRoot) {
        NSString *path = [[NSBundle mainBundle]pathForResource:@"monthPlist" ofType:@"plist"];
