...
- (void)selectedOk
{
    if (self.selectDateStr.length == 0)
    {
        ...
        {
            self.selectDateStr = [dateFormatter stringFromDate:self.customDate];
        }
        else
        {
            self.selectDateStr = [dateFormatter stringFromDate:[NSDate date]];
        }
    }
    else
    {
        self.selectDateStr = self.selectDateStr;
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(MyDatePicker:SelectedDateStr:)])
    {
        [self.delegate MyDatePicker:self.pickerView SelectedDateStr:self.selectDateStr];
    }
    [self remove];
}
...
- (UIDatePicker *)pickerView
{
    if (!_pickerView) {
        ...
        _pickerView = [[UIDatePicker alloc]initWithFrame:CGRectMake(pickerX, pickerY, pickerW, pickerH)];
        _pickerView.locale = [NSLocale localeWithLocaleIdentifier:@"zh"];
        _pickerView.datePickerMode = UIDatePickerModeDate;
        [_pickerView addTarget:self action:@selector(datePickerChange:) forControlEvents:UIControlEventValueChanged];
