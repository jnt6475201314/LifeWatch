...
@interface ReportMonthPicker ()<UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation ReportMonthPicker
...
- (void)setupUI
{
    ...
    [self.arrayRoot enumerateObjectsUsingBlock:^(NSDictionary *obj, NSUInteger idx, BOOL * _Nonnull stop) {
        [self.arrayStartMonth addObject:obj[@"year"]];
    }];
    
    self.arrayEndMonth= [NSMutableArray arrayWithArray:[self.arrayRoot firstObject][@"month"]];
    ...
    [self setTitle:Report_SelectTime_Month];
...
{
    if (component == 0) {
        self.arraySelected = self.arrayRoot[row][@"month"];
...
- (void)selectedOk
{
    [self.delegate ReportPicker:self selectedYear:self.monthStartStr selectedMonth:self.monthEndStr];
    [super selectedOk];
}
...
{
    if (!_arrayRoot) {
        NSString *path = [[NSBundle mainBundle]pathForResource:KChineseStyle ?@"ReportMonthPlist":@"ReportMonthPlist_en" ofType:@"plist"];
