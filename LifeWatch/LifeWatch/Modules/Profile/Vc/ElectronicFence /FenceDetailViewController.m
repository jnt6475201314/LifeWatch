...
@interface FenceDetailViewController ()<UITableViewDelegate, UITableViewDataSource,BMKMapViewDelegate, BMKLocationServiceDelegate, CLLocationManagerDelegate, BMKOverlay, UITextFieldDelegate, BMKGeoCodeSearchDelegate, PickerAreaDelegate>
{
...
- (void)configUI{
    ...
    NSLog(@"%@", self.dict);
    userInfoArr = [self.dict[@"FenceUser"] componentsSeparatedByString:@"|"];
    _latitude = self.dict[@"CenterLatitude"];
    _longitude = self.dict[@"CenterLongitude"];
    _address = self.dict[@"Address"];

    _radius = [self.dict[@"Radius"] stringValue];
    _noOutRange = [self.dict[@"DataType"] intValue]==1?YES:NO;
    ...
    _circle = [BMKCircle circleWithCenterCoordinate:location.coordinate radius:[_radius doubleValue]];
...
- (UIView *)configTableFooterV
{
    ...
    UIButton * _deleteBtn = [[UIButton alloc] initWithFrame:CGRectMake(20, 10, kScreenWidth-40, 40)];
    _deleteBtn.backgroundColor = KRedColor;
    [_deleteBtn setTitle:GeoFence_Delete forState:UIControlStateNormal];
    ...
    [_footerV addSubview:_deleteBtn];
...
- (void)saveButtonEvent
{
    ...
    NSDictionary * params = @{@"method":@"SaveElecFence",@"user_id":KGetUserID,@"fence_userid":self.dict[@"FenceUserId"], @"elec_id":self.dict[@"Id"], @"address":_address, @"longitude":_longitude, @"latitude":_latitude, @"radius":_radius, @"enabled":self.dict[@"Enabled"], @"data_type":_noOutRange?@"1":@"2", @"action":@"edit"};
...
{
    NSString * url = KSaveElecFenceUrl;
    NSDictionary * params = @{@"method":@"SaveElecFence",@"user_id":KGetUserID,@"fence_userid":self.dict[@"FenceUserId"], @"elec_id":self.dict[@"Id"], @"address":_address, @"longitude":_longitude, @"latitude":_latitude, @"radius":_radius, @"enabled":@"1", @"data_type":_noOutRange?@"1":@"2", @"action":@"delete"};
    [NetRequest postUrl:url Parameters:params success:^(NSDictionary *resDict) {
        NSLog(@"%@", resDict);
        if ([resDict[@"result"] integerValue] == 1) {
            [self showHUD:resDict[@"msg"] de:1.0];
...
{
    NSString * _latitudeStr = userInfoArr[7];
    NSString * _longitudeStr = userInfoArr[8];
    CLLocation *location = [[CLLocation alloc] initWithLatitude:[_latitudeStr doubleValue] longitude:[_longitudeStr doubleValue]];
    [self addPointAnnotationWith:location address:userInfoArr[9]];
    [_mapView setCenterCoordinate:location.coordinate];// 当前地图的中心点
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (NULL_TO_NIL(self.dict[@"FenceUser"])!=nil) {
        ...
        if (indexPath.row == 0) {
            ...
            _nameLab.text = [NSString stringWithFormat:@"%@",userInfoArr[1]];
            ...
            NSString * time = self.dict[@"CreateDate"];
