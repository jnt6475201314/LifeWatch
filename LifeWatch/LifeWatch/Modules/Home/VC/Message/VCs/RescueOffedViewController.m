...
@interface RescueOffedViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation RescueOffedViewController
...
- (void)loadData{
    ...
    NSString * url = KMySOSListUrl; // state 状态：0=新创建； 1=已撤销； 2=救援进行中； 5=救援结束
    NSDictionary * params = @{@"method":@"MySOSList",@"user_id":KGetUserID,  @"state":self.state, @"page_size":@"10", @"page_index":@"1"};
...
- (void)loadMoreData
{
    ...
    NSDictionary * params = @{@"method":@"MySOSList",@"user_id":KGetUserID,  @"state":self.state, @"page_size":@"10", @"page_index":@(self.page_index)};
    [NetRequest postUrl:url Parameters:params success:^(NSDictionary *resDict) {
        NSLog(@"%@", resDict);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count != 0) {
        ...
        if (NULL_TO_NIL(user1Dict[@"UserInfo"])!=nil) {
            ...
            NSLog(@"%@", userInfoArr);
            cell.contentLab.text = [NSString stringWithFormat:@"%@:%@，%@：%@， %@：%@，%@：%@， %@：%@", MyNews_Rescuer, userInfoArr[1], MyNews_Time, time, MyNews_Category, _DataTypeStr, MyNews_Location, self.dataArray[indexPath.section][@"UserAddress"], MyNews_Status, _statusStr];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MyRescueDetailViewController * vc = [[MyRescueDetailViewController alloc] init];
