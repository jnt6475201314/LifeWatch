//
//  DetailForMonthViewController.m
//  LifeWatch
//
...
@interface DetailForMonthViewController ()<UITableViewDelegate, UITableViewDataSource, ReportMonthPickerDelegate>
{
...
@implementation DetailForMonthViewController
...
- (void)loadDataWithMonth:(NSString *)month{
    ...
    NSDictionary * params = @{@"method":@"GetHealthData", @"data_type":self.data_type, @"user_id":KGetUserID, @"date_type":@"2", @"date":month};
...
- (LineChartData *)setWithChartView:(DetailChartView *)chartView DataWithMaxYVal:(double)maxYVal data:(NSArray *)dataArray labelName:(NSString *)labelName{
    ...
    }else
    {
        ...
        NSMutableArray *xVals = [[NSMutableArray alloc] init];
        for (int i = 0; i < xVals_count; i++) {
            [xVals addObject:[NSString stringWithFormat:@"%d:00", [dataArray[i][@"x"] intValue]]];
        }
        chartView.xAxisArray = xVals;
...
//    NSLog(@"%@", dataContent);
    ...
    if (indexPath.section == 0) {
        ...
        _dateLab.text = [NSString stringWithFormat:@"%@-%@", query_date, self.resultDict[@"rows"][indexPath.row][@"x"]];
...
-(NSDictionary *)dataDict
{
    if (!_dataDict) {
        _dataDict = @{
                      @"sleep":@{@"data_type":@"sleep", @"formatterType":@"3", @"maxYVal":@"12", @"labelName":[NSString stringWithFormat:@"%@ %@：%@", Data_Sleep_data, Goal_Unit, Report_hours],
                                         @"limit":@[@"8", @"6"]},
